...
2. БД: из `docker/infra` поднять `leads-db`, затем применить `migrations/001_init_leads.sql`, `migrations/002_add_myhome_listing_fields.sql`, `migrations/003_add_lead_details.sql`, `**migrations/004_add_text_lang_columns.sql**`, `**migrations/005_myhome_api_first.sql**` и `**migrations/006_add_price_gel_rename_price_usd.sql**` к БД на `localhost:5433` (порядок: **006** после **005**; **`price_total_usd`** в схеме заменён на **`price_usd`**, добавлена **`price_gel`**).
