...
Проверка compose: `docker compose -f docker/infra/docker-compose.yml config` (аналогично для `docker/tools`, `docker/app`, `docker/reverse-proxy`). Деплой на сервер: `[docs/DEPLOY_SERVER.md](docs/DEPLOY_SERVER.md)`.
