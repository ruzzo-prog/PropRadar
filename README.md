...
├── compose.yaml              # Единая точка входа Docker Compose (профили infra / app / tools / proxy)
├── docker/
│   ├── infra/                # PostgreSQL 15 (leads-db, хост-порт 5433)
│   ├── tools/                # n8n, Metabase:3031, Evolution API:8080
│   ├── reverse-proxy/        # Nginx TLS для n8n/Evolution (см. README в папке)
│   └── app/                  # Каркас parsers + FastAPI (Docker: api :8000)
├── pyproject.toml
├── CHANGELOG.md
└── README.md
```
...
2. БД: из корня репозитория `docker compose --profile infra up -d leads-db` (или из `docker/infra`: `docker compose --profile infra up -d`), затем применить `migrations/001_init_leads.sql`, `migrations/002_add_myhome_listing_fields.sql`, `migrations/003_add_lead_details.sql`, `**migrations/004_add_text_lang_columns.sql**`, `**migrations/005_myhome_api_first.sql**`, `**migrations/006_add_price_gel_rename_price_usd.sql**`, `**migrations/007_create_leads_client_table.sql**`, `**migrations/008_recreate_leads_client_v2.sql**` и `**migrations/009_add_city_name_to_leads_client.sql**`, `**migrations/010_add_status_reason_to_leads.sql**`, `**migrations/011_add_phone_retries.sql**` к БД на `localhost:5433` (порядок: **006** после **005**, **007** после **006**, **008** после **007**, **009** после **008**, **010** после **009**, **011** после **010**; `**price_total_usd`** в схеме заменён на `**price_usd**`, добавлена `**price_gel**`; **007**+**008**+**009** — таблица `**leads_client`**: после **008** PK `**(source, external_id)**`, проекция для UI/Metabase, синхронизация триггером из `**leads**`; **009** добавляет **`city_name`** / **`owner_name`** в проекцию из **`myhome_statement_json`**).
