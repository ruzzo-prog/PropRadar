...
2. БД: из `docker/infra` поднять `leads-db`, затем применить `migrations/001_init_leads.sql`, `migrations/002_add_myhome_listing_fields.sql`, `migrations/003_add_lead_details.sql`, `**migrations/004_add_text_lang_columns.sql**`, `**migrations/005_myhome_api_first.sql**`, `**migrations/006_add_price_gel_rename_price_usd.sql**`, `**migrations/007_create_leads_client_table.sql**`, `**migrations/008_recreate_leads_client_v2.sql**` и `**migrations/009_add_city_name_to_leads_client.sql**` к БД на `localhost:5433` (порядок: **006** после **005**, **007** после **006**, **008** после **007**, **009** после **008**; `**price_total_usd`** в схеме заменён на `**price_usd**`, добавлена `**price_gel**`; **007**+**008**+**009** — таблица `**leads_client`**: после **008** PK `**(source, external_id)**`, проекция для UI/Metabase, синхронизация триггером из `**leads**`; **009** добавляет **`city_name`** / **`owner_name`** в проекцию из **`myhome_statement_json`**).
