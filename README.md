...
2. БД: из `docker/infra` поднять `leads-db`, затем применить `migrations/001_init_leads.sql`, `migrations/002_add_myhome_listing_fields.sql`, `migrations/003_add_lead_details.sql`, `**migrations/004_add_text_lang_columns.sql**`, `**migrations/005_myhome_api_first.sql**`, `**migrations/006_add_price_gel_rename_price_usd.sql**`, `**migrations/007_create_leads_client_table.sql**` и `**migrations/008_recreate_leads_client_v2.sql**` к БД на `localhost:5433` (порядок: **006** после **005**, **007** после **006**, **008** после **007**; `**price_total_usd`** в схеме заменён на `**price_usd**`, добавлена `**price_gel**`; **007**+**008** — таблица `**leads_client`**: после **008** PK `**(source, external_id)**`, проекция для UI/Metabase, синхронизация триггером из `**leads**`).
