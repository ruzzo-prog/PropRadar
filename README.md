...
2. БД: из `docker/infra` поднять `leads-db`, затем применить `migrations/001_init_leads.sql`, `migrations/002_add_myhome_listing_fields.sql`, `migrations/003_add_lead_details.sql`, `**migrations/004_add_text_lang_columns.sql**`, `**migrations/005_myhome_api_first.sql**`, `**migrations/006_add_price_gel_rename_price_usd.sql**` и `**migrations/007_create_leads_client_table.sql**` к БД на `localhost:5433` (порядок: **006** после **005**, **007** после **006**; `**price_total_usd`** в схеме заменён на `**price_usd**`, добавлена `**price_gel**`; **007** — таблица `**leads_client`**: проекция для UI/Metabase, синхронизация триггером из `**leads**`).
...
8. Дашборд Metabase через API (после первого входа и подключения БД **«PropRadar Leads»**): `python scripts/setup_metabase_dashboard.py` (переменные `**METABASE_*`** в `.env`). SQL в **`metabase/propradar_dashboard.json`** рассчитан на проекцию **`leads_client`** (миграция **007** после **006**); операторские заметки по вёрстке таблицы на дашборде — в **`docs/METABASE_SETUP.md`** и в поле **`operator_instructions_ru`** JSON.
