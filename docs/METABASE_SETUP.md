...
**Smoke (человек):** 10 плиток на мониторинге, график с двумя сериями, карта с точками в Грузии, запросы без ошибки PostgreSQL (миграции **007–009** для `owner_name` / `urban_name`). Таблица последних лидов: этаж без `N/N`, «Состояние» — поле **condition** (не `comment`). Дашборд карты: 2 скаляра сверху; pin использует swapped `map.*_column` из bundle.
