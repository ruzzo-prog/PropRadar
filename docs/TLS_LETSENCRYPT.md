...
Это относится ко **всем** доменам: **n8n**, **Evolution**, **Metabase**, **Snapotter**.
...
```bash
sudo certbot certonly --standalone --email <YOUR_EMAIL> --agree-tos --no-eff-email -d snapotter.usluga-market.ru
```
...
## 4. Переменные `.env` на хосте (все домены)

Compose (фрагмент `docker/reverse-proxy/docker-compose.yml`) монтирует **отдельные файлы** PEM в **фиксированные** пути внутри контейнера. На хосте задаются **восемь** переменных:
...
| **`SNAPOTTER_TLS_FULLCHAIN`** | `/etc/nginx/certs/snapotter/fullchain.pem` |
| **`SNAPOTTER_TLS_PRIVKEY`** | `/etc/nginx/certs/snapotter/privkey.pem` |
...
- Перед **`nginx`** в **`command`** выполняется **`00-tls-preflight.sh`**: проверка **`-e`**, **`-f`**, **`-r`** для каждого из восьми путей. Любая ошибка → **exit 1**, контейнер не стартует, в логах текст с перечнем переменных.
...
## 7. Контур Snapotter (текущая схема)

- Образ **`snapotter/snapotter:latest`**, сервис **`snapotter`** в **`docker/app/docker-compose.yml`**, профиль **`app`**, **`ANALYTICS_ENABLED=false`**, том **`snapotter_data`**, слушает **1349** внутри сети; на хост порт **не пробрасывается**.
- Reverse-proxy проксирует **`snapotter.usluga-market.ru`** → **`snapotter:1349`**.
- После добавления PEM в `.env`: поднять приложение **`docker compose --profile app up -d snapotter`** (при необходимости вместе с **`infra`** и зависимостями), затем пересоздать **`reverse-proxy`** (команда выше).

---

## 8. Ограничения и несоответствия по умолчанию
...
- **Профили:** для API+БД обычно нужны **`--profile infra --profile app`**; для Snapotter с прокси — минимум **`app`** + **`proxy`** при уже существующей сети и зависимостях (см. известные ограничения в `docs/PropRadar_STATUS.md`).

---

## 9. Верификация после изменений
